    ...
    class Tax_Meta_Class {
        ...
        public function insert_images() {
            ...
            foreach ($_POST['attachments'] as $attachment_id => $attachment) {
                ...
                $li .= "<a title='" . esc_html__('Delete this image', 'sailing') . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . esc_url($this->SelfPath) . "/images/delete-16.png' alt='" . esc_attr__('Delete', 'sailing') . "' /></a>";
        ...
        public function wp_ajax_delete_image() {
            ...
            if ($ok) {
                ...
                echo json_encode(array('message' => esc_html__('Cannot delete file. Something\'s wrong.', 'sailing')));
                die();
            }
        }
        ...
        <img src="';
                    ...
                    echo '" alt="' . esc_html__('Remove', 'sailing') . '" title="' . esc_html__('Remove', 'sailing') . '" id="remove-' . $field['id'] . '"></div>';
                    $c = $c + 1;
                }
            ...
            echo '" alt="' . esc_html__('Add', 'sailing') . '" title="' . esc_html__('Add', 'sailing') . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
        ...
        public function show_field_file($field, $meta) {
            ...
            if (!empty($meta) && count($meta) > 0 && !$this->is_array_empty($meta)) {
                ...
                echo '<div style="margin-bottom: 10px"><strong>' . esc_html__('Uploaded files', 'sailing') . '</strong></div>';
                echo '<ol class="at-upload">';
                foreach ($meta as $att) {
                    // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
                    echo "<li>" . wp_get_attachment_link($att, '', false, false, ' ') . " (<a class='at-delete-file' href='#' rel='{$nonce}||{$field['id']}|{$att}'>" . esc_html__('Delete', 'sailing') . "</a>)</li>";
                }
                echo '</ol>';
            }
      ...
      <strong>" . esc_html__('Upload new files', 'sailing') . "</strong>\n
    </div>\n";
            echo "<div class='new-files'>\n
      <div class='file-input'>\n
        <input type='file' name='{$field['id']}[]' />\n
      </div><!-- End .file-input -->\n
      <a class='at-add-file button' href='#'>" . esc_html__('Add more files', 'sailing') . "</a>\n
      </div><!-- End .new-files -->\n";
            echo "</td>";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_color($field, $meta) {
            ...
            echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_html__('Select a color', 'sailing') . "'/>";
